...
var AlignmentPatternFinder = module.exports = function(
  ...
  this.crossCheckVertical = function(
    startI, centerJ, maxCount, originalStateCountTotal) {
...
module.exports = function BitMatrixParser(bitMatrix) {
  ...
  if (dimension < 21 || (dimension & 0x03) != 1) {
    throw new Error("Invalid BitMatix dimension");
  }
...
DataBlock.getDataBlocks = function(rawCodewords, version, ecLevel) {
  ...
  if (rawCodewords.length != version.TotalCodewords) {
    throw new Error("Different number of code words from given version");
  }
  ...
  // Now establish DataBlocks of the appropriate size and number of codewords
  ...
  for (var j = 0; j < ecBlockArray.length; j++) {
    ...
    for (var i = 0; i < ecBlock.Count; i++) {
      ...
      result[numResultBlocks++] =
        new DataBlock(numDataCodewords, new Array(numBlockCodewords));
    }
  }
...
module.exports = function QRCodeDataBlockReader(
  ...
  this.getNextBits = function(numBits) {
    ...
    if (numBits < this.bitPointer + 1) {
      ...
      bits = (this.blocks[this.blockPointer] & mask) >>
      ...
      bits = (this.blocks[this.blockPointer] & mask1) <<
        (numBits - (this.bitPointer + 1));
      this.blockPointer++;
      bits += ((this.blocks[this.blockPointer]) >>
      ...
      //8 : number of the 2nd block
        // (note that use already 8bits because next word uses 3 data blocks)
      //numBits - (bitPointer + 1 + 8) : number of bits of the 3rd block
      for (var i = 0; i < this.bitPointer + 1; i++) {
        mask1 += (1 << i);
      }
      var bitsFirstBlock = (this.blocks[this.blockPointer] & mask1) <<
        (numBits - (this.bitPointer + 1));
      this.blockPointer++;

      var bitsSecondBlock = this.blocks[this.blockPointer] <<
      ...
      var bitsThirdBlock = (this.blocks[this.blockPointer] & mask3) >>
  ...
  this.getKanjiString = function(dataLength) {
    ...
    do {
      ...
      //unicodeString += new String(SystemUtils.ToCharArray(
        //SystemUtils.ToByteArray(tempByte)));
  ...
  this.__defineGetter__("DataByte", function() {
    ...
    do {
      ...
      if (mode != MODE_NUMBER && mode != MODE_ROMAN_AND_NUMBER &&
        ...
        throw new Error(
          "Invalid mode: " + mode + " in (block:" +
            this.blockPointer + " bit:" + this.bitPointer + ")");
      }
      ...
      switch (mode) {
        ...
        case MODE_ROMAN_AND_NUMBER:
          ...
          for (var j = 0; j < temp_str.length; j++)
            ta[j] = temp_str.charCodeAt(j);
          output.push(ta);
          //output.Write(SystemUtils.ToByteArray(temp_sbyteArray2),
            //0, temp_sbyteArray2.Length);
          break;
        ...
        case MODE_8BIT_BYTE:
          ...
          //output.Write(SystemUtils.ToByteArray(temp_sbyteArray3),
            //0, temp_sbyteArray3.Length);
          break;

        case MODE_KANJI:
          //canvas.println("Mode: Kanji");
          //sbyte[] temp_sbyteArray4;
          //temp_sbyteArray4 = SystemUtils.ToSByteArray(
            //SystemUtils.ToByteArray(getKanjiString(dataLength)));
          //output.Write(SystemUtils.ToByteArray(temp_sbyteArray4),
...
Decoder.correctErrors = function(codewordBytes, numDataCodewords) {
  ...
  // Copy back into array of bytes -
  // only need to worry about the bytes that were data
...
var PerspectiveTransform = module.exports.PerspectiveTransform = function(
...
PerspectiveTransform.quadrilateralToQuadrilateral = function(
  ...
  var qToS = this.quadrilateralToSquare(
...
module.exports.Detector = function(image) {
  ...
  this.calculateModuleSizeOneWay = function(pattern, otherPattern) {
    var moduleSizeEst1 = this.sizeOfBlackWhiteBlackRunBothWays(
      Math.floor(pattern.X), Math.floor(pattern.Y),
      Math.floor(otherPattern.X), Math.floor(otherPattern.Y));
    ...
    // black modules, and 1 white and 1 black module on either side.
    // Ergo, divide sum by 14.
  ...
  this.findAlignmentInRegion = function(overallEstModuleSize,
    estAlignmentX, estAlignmentY, allowanceFactor) {

    // Look for an alignment pattern (3 modules in size) around where it
    // should be
    var allowance = Math.floor(allowanceFactor * overallEstModuleSize);
    var alignmentAreaLeftX =
      Math.max(0, estAlignmentX - allowance);
    var alignmentAreaRightX =
      Math.min(qrcode.width - 1, estAlignmentX + allowance);
    if (alignmentAreaRightX - alignmentAreaLeftX < overallEstModuleSize * 3) {
      throw new Error();
    }

    var alignmentAreaTopY =
      Math.max(0, estAlignmentY - allowance);
    var alignmentAreaBottomY =
      Math.min(qrcode.height - 1, estAlignmentY + allowance);

    var alignmentFinder = new AlignmentPatternFinder(
      this.image, alignmentAreaLeftX, alignmentAreaTopY,
      alignmentAreaRightX - alignmentAreaLeftX,
      alignmentAreaBottomY - alignmentAreaTopY,
      overallEstModuleSize, this.resultPointCallback);
  ...
  this.createTransform = function(
    ...
    var transform = PerspectiveTransform.quadrilateralToQuadrilateral(
      3.5, 3.5, dimMinusThree, 3.5,
      sourceBottomRightX, sourceBottomRightY, 3.5, dimMinusThree,
  ...
  this.processFinderPatternInfo = function(info) {
    ...
    var dimension = this.computeDimension(
      topLeft, topRight, bottomLeft, moduleSize);
      ...
      for (var i = 4; i <= 16; i <<= 1) {
        try {
          alignmentPattern = this.findAlignmentInRegion(
            moduleSize, estAlignmentX, estAlignmentY, i);
          break;
        }
        catch (re) {
          // try next round
...
module.exports = function() {
  ...
  this.foundPatternCross = function(stateCount) {
    ...
    return Math.abs(
      moduleSize - (stateCount[0] << INTEGER_MATH_SHIFT)) < maxVariance &&
      Math.abs(
        moduleSize - (stateCount[1] << INTEGER_MATH_SHIFT)) < maxVariance &&
      Math.abs(
        3 * moduleSize - (stateCount[2] << INTEGER_MATH_SHIFT)) <
        3 * maxVariance &&
      Math.abs(
        moduleSize - (stateCount[3] << INTEGER_MATH_SHIFT)) < maxVariance &&
      Math.abs(
        moduleSize - (stateCount[4] << INTEGER_MATH_SHIFT)) < maxVariance;
  };
  this.centerFromEnd = function(stateCount, end) {
  ...
  this.crossCheckHorizontal = function(
    startJ, centerI, maxCount, originalStateCountTotal) {
  ...
  this.handlePossibleCenter = function(stateCount, i, j) {
    ...
    var centerI = this.crossCheckVertical(
      i, Math.floor(centerJ),
      stateCount[2], stateCountTotal); //float
    if (!isNaN(centerI)) {
      // Re-cross check
      centerJ = this.crossCheckHorizontal(
        Math.floor(centerJ), Math.floor(centerI),
  ...
  this.selectBestPatterns = function() {
    ...
    if (startSize > 3) {
      ...
      // at least 4 possibilities to choose from
  ...
  this.haveMultiplyConfirmedCenters = function() {
    ...
    // that when the total deviation from average exceeds 5% of the total
    // module size estimates, it's too much.
  ...
  this.findFinderPattern = function(image) {
    ...
    //console.log(maxI, maxJ, iSkip, MIN_SKIP, tryHarder,
      //qrcode.height, qrcode.width, qrcode.asdf);
                ...
                if (confirmed) {
                  ...
                  if (this.hasSkipped) {
                    ...
                    if (rowSkip > stateCount[2]) {
                      ...
                      // (size of last center of pattern we saw) to be
                      // conservative, and also back off by iSkip which
...
var GF256Poly = module.exports = function(field, coefficients) {
  ...
  if (coefficientsLength > 1 && coefficients[0] == 0) {
    // Leading term must be non-zero for anything
    // except the constant polynomial "0"
    var firstNonZero = 1;
    ...
    if (firstNonZero == coefficientsLength) {
      ...
      //Array.Copy(coefficients, firstNonZero,
  ...
  this.evaluateAt = function(a) {
    ...
    for (var i = 1; i < size; i++) {
      result2 = GF256.addOrSubtract(
        this.field.multiply(a, result2), this.coefficients[i]);
    }
    return result2;
  };
  ...
  this.addOrSubtract = function(other) {
    ...
    // Copy high-order terms only found in
    // higher-degree polynomial's coefficients
  ...
  this.divide = function(other) {
    ...
    var inverseDenominatorLeadingTerm =
      this.field.inverse(denominatorLeadingTerm);
    ...
    while (remainder.Degree >= other.Degree && !remainder.Zero) {
      ...
      var iterationQuotient =
        this.field.buildMonomial(degreeDifference, scale);
...
GridSampler.checkAndNudgePoints = function(image, points) {
  ...
  for (var offset = 0; offset < points.length && nudged; offset += 2) {
    ...
    if (x < -1 || x > width || y < -1 || y > height) {
      throw new Error("Coordinate out of range: " + x + "," + y);
    }
    nudged = false;
    if (x == -1) {
      points[offset] = 0.0;
...
GridSampler.sampleGrid3 = function(image, dimension, transform) {
  ...
  for (var y = 0; y < dimension; y++) {
    ...
    try {
      for (var x = 0; x < max; x += 2) {
        var xpoint = (Math.floor(points[x]) * 4) +
          (Math.floor(points[x + 1]) * qrcode.width * 4);
        var bit = image[Math.floor(points[x]) +
      ...
      // maps a straight line of points to a set of points whose endpoints are
      // in bounds, but others are not. There is probably some mathematical
      // way to detect this about the transformation that I don't know yet.
      // This results in an ugly runtime exception despite our clever checks
      // above -- can't have that. We could check each point's coordinates but
      // that feels duplicative. We settle for catching and wrapping
      // ArrayIndexOutOfBoundsException.
      throw new Error("Twisted transform error, too lazy to handle it");
    }
  }
  return bits;
};

GridSampler.sampleGridx = function(image, dimension,
  ...
  return new Promise(function(resolve) {
    if (src == null) {
      ...
      image.onload = function() {
        ...
        try {
          qrcode.imagedata = context.getImageData(
            0, 0, canvas_qr.width, canvas_qr.height);
        }
        catch (e) {
          qrcode.result = null;
          throw new Error(
...
qrcode.getPixel = function(x, y) {
  ...
  var point = (x * 4) + (y * qrcode.width * 4);
  var p = (qrcode.imagedata.data[point] * 33 +
    ...
    for (var ax = 0; ax < numSqrtArea; ax++) {
      ...
      for (var dy = 0; dy < areaHeight; dy++) {
        for (var dx = 0; dx < areaWidth; dx++) {
          var target = image[areaWidth * ax + dx +
...
qrcode.grayScaleToBitmap = function(grayScale) {
  ...
  for (var ay = 0; ay < sqrtNumArea; ay++) {
    for (var ax = 0; ax < sqrtNumArea; ax++) {
      for (var dy = 0; dy < areaHeight; dy++) {
        for (var dx = 0; dx < areaWidth; dx++) {
          bitmap[areaWidth * ax + dx +
            (areaHeight * ay + dy) * qrcode.width] =
            (grayScale[areaWidth * ax + dx +
              (areaHeight * ay + dy) * qrcode.width] <
                middle[ax][ay]) ? true : false;
        }
      }
    }
  }
...
qrcode.orderBestPatterns = function(patterns) {
  ...
  // Assume one closest to other two is B;
  // A and C will just be guesses at first
...
module.exports = function(field) {
  ...
  this.decode = function(received, twoS) {
    ...
    var errorMagnitudes = this.findErrorMagnitudes(
      omega, errorLocations, dataMatrix);
  ...
  this.runEuclideanAlgorithm = function(a, b, R) {
    ...
    while (r.Degree >= Math.floor(R / 2)) {
      ...
      while (r.Degree >= rLast.Degree && !r.Zero) {
        ...
        var scale = this.field.multiply(
          r.getCoefficient(r.Degree), dltInverse);
  ...
  this.findErrorMagnitudes = function(
    errorEvaluator, errorLocations, dataMatrix) {
      ...
      for (var j = 0; j < s; j++) {
        if (i != j) {
          denominator = this.field.multiply(
            denominator, GF256.addOrSubtract(
              1, this.field.multiply(errorLocations[j], xiInverse)));
        }
      }
      result[i] = this.field.multiply(
        errorEvaluator.evaluateAt(xiInverse), this.field.inverse(denominator));
      // Thanks to sanfordsquires for this fix:
      if (dataMatrix) {
...
var Version = module.exports = function(versionNumber, alignmentPatternCenters,
...
Version.getVersionForNumber = function(versionNumber) {
  if (versionNumber < 1 || versionNumber > 40) {
    throw new Error("Invalid version: " + versionNumber);
  }
